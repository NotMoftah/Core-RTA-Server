...
namespace CoreTcp
{
    ...
    public class TcpClient : IDisposable
    {
        ...
        public TcpClient(IPAddress address, int port, string token) : this(new IPEndPoint(address, port), token) { }
        /// <summary>
        /// Initialize TCP client with a given server IP address and port number
        /// </summary>
        /// <param name="address">IP address</param>
        /// <param name="port">Port number</param>
        public TcpClient(string address, int port, string token) : this(new IPEndPoint(IPAddress.Parse(address), port), token) { }
        /// <summary>
        /// Initialize TCP client with a given IP endpoint
        /// </summary>
        /// <param name="endpoint">IP endpoint</param>
        public TcpClient(IPEndPoint endpoint, string token)
        {
            Endpoint = endpoint;
            Token = token.PadRight(64, 'K').Substring(0, 64);
        }
        ...
        public virtual bool Connect()
        {
            ...
            // Send the token
            SendAsync(Encoding.UTF8.GetBytes(Token));
